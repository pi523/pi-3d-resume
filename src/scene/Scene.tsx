...
function Man2({
  ...
  // 程序化摸猫：手部球形区域的顶点沿 dir 往复位移（vertex shader 内完成）。
  // center/dir 为 man 本地系（用 /debug.html 点击拾取）；radius 影响范围、amp 单侧行程。
  const PET = {
    enabled: false, // 程序化版观感不佳已停用，改走 Blender 骨骼动画（烘焙进 glb）
    center: [-0.264, -0.437, 0.468] as [number, number, number],
    dir: [0.96, 0.26, -0.14] as [number, number, number],
    radius: 0.17,
    amp: 0.045,
    scrollSpeed: 0.3, // 每滚动帧的相位增量（rad）
    idleSpeed: 1.4, // 待机相位速度（rad/s）
  }
  ...
  const { model, eyes, followGroup, petUniforms, points, startPoint, glbCam, focusNode, dof } = useMemo(() => {
    ...
    // 贴纸：按配置生成小平面贴到 man 节点下（跟随人物动画），法线定朝向
    const manNode = (() => {
      let m: any = null
      clone.traverse((o: any) => {
        if (o.name === 'man') m = o
    ...
    // 转头跟随用的包裹组：man 的局部变换被 manAction 动画驱动，直接改会被覆盖/累积，
    // 故插入一层独立父组，每帧对它设置绝对角度。
    let followGroup: any = null
    if (manNode && manNode.parent) {
      followGroup = new THREE.Group()
      followGroup.name = 'man-follow'
      manNode.parent.add(followGroup)
      followGroup.add(manNode)
    }
    // 摸猫 shader 注入：把 man 本地系配置换算到 body 网格几何本地系，改写材质顶点着色器
    let petUniforms: any = null
    if (manNode && PET.enabled) {
      let bodyMesh: any = null
      clone.traverse((o: any) => {
        if (o.isMesh && o.name === 'body') bodyMesh = o
      })
      if (bodyMesh) {
        clone.updateMatrixWorld(true)
        const toBody = (v: [number, number, number]) =>
          bodyMesh.worldToLocal(manNode.localToWorld(new THREE.Vector3(...v)))
        const c = toBody(PET.center)
        const d = toBody([
          PET.center[0] + PET.dir[0],
          PET.center[1] + PET.dir[1],
          PET.center[2] + PET.dir[2],
        ])
          .sub(c)
          .normalize()
        const bodyScale = bodyMesh.getWorldScale(new THREE.Vector3()).x || 1
        petUniforms = {
          uPetCenter: { value: c },
          uPetDir: { value: d },
          uPetRadius: { value: PET.radius / bodyScale },
          uPetAmp: { value: PET.amp / bodyScale },
          uPetPhase: { value: 0 },
        }
        const mat = bodyMesh.material
        mat.onBeforeCompile = (sh: any) => {
          Object.assign(sh.uniforms, petUniforms)
          sh.vertexShader = sh.vertexShader
            .replace(
              '#include <common>',
              '#include <common>\nuniform vec3 uPetCenter; uniform vec3 uPetDir; uniform float uPetRadius; uniform float uPetAmp; uniform float uPetPhase;'
            )
            .replace(
              '#include <begin_vertex>',
              '#include <begin_vertex>\n' +
                'float petW = 1.0 - smoothstep(uPetRadius * 0.35, uPetRadius, distance(position, uPetCenter));\n' +
                'transformed += uPetDir * (sin(uPetPhase) * uPetAmp * petW);'
            )
        }
        mat.needsUpdate = true
      }
    }
    if (manNode && STICKERS.length) {
      const texLoader = new THREE.TextureLoader()
      // 贴花投影（DecalGeometry）：把贴纸网格真正裹在脸部曲面上，不悬浮不陷面。
      // 人物是单张蒙皮网格（SkinnedMesh），DecalGeometry 只认「几何 + matrixWorld」、
      // 不算骨骼变换，直接投影会落在绑定姿态上（与可见表面错位 → 贴花为空）。
      // 因此先把蒙皮后的顶点烘焙成世界空间静态网格，对烘焙网格做射线与投影。
      clone.updateMatrixWorld(true)
      const baked: any[] = []
      manNode.traverse((o: any) => {
        if (!o.isMesh) return
      ...
      const invMan = new THREE.Matrix4().copy(manNode.matrixWorld).invert()
      for (const st of STICKERS) {
        const tex = texLoader.load(`${import.meta.env.BASE_URL}${st.img}`)
        tex.colorSpace = THREE.SRGBColorSpace
        tex.anisotropy = 16 // 斜视角下防糊
        const nLocal = new THREE.Vector3(...st.n).normalize()
        const pLocal = new THREE.Vector3(...st.p)
        // 从贴点上方沿 -n 打射线，拿到真实表面点与所属网格
        const origin = manNode.localToWorld(pLocal.clone().addScaledVector(nLocal, 0.05))
        const nWorld = nLocal.clone().transformDirection(manNode.matrixWorld).normalize()
        ray.set(origin, nWorld.clone().negate())
        const hit = ray.intersectObjects(baked, false)[0]
        if (!hit) continue
        const orient = new THREE.Euler().setFromQuaternion(
          new THREE.Quaternion()
            .setFromUnitVectors(new THREE.Vector3(0, 0, 1), nWorld)
            .multiply(
              new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 0, 1), (st.roll * Math.PI) / 180)
            )
        )
        // 投影盒深度要盖住曲面起伏，太深会裹到别的部位
        const geo = new DecalGeometry(
          hit.object as any,
          hit.point,
          orient,
          new THREE.Vector3(st.size, st.size, Math.max(st.size, 0.04))
        )
        // DecalGeometry 输出世界坐标（烘焙网格矩阵为单位阵）→ 转回 man 本地，挂回 manNode 跟随模型
        geo.applyMatrix4(invMan)
        // 受光材质：贴纸跟随脸部明暗（MeshBasicMaterial 恒定全亮会显得浮贴）
        ...
        // 场景 key+fill+hemi 总光强 >3，贴图色被乘爆再经 ACES → logo 洗白。
        // 压亮不压暗：阴影照常（保留贴合感），高光最多提亮 25%，印刷色保持可读。
        mat.onBeforeCompile = (sh: any) => {
          sh.fragmentShader = sh.fragmentShader.replace(
            'vec3 outgoingLight = totalDiffuse + totalSpecular + totalEmissiveRadiance;',
            'vec3 outgoingLight = min(totalDiffuse + totalSpecular + totalEmissiveRadiance, diffuseColor.rgb * 1.25);'
          )
        }
      ...
      baked.forEach((b) => b.geometry.dispose())
    }
    const eyes: any[] = []
    ...
    clone.traverse((o: any) => {
      if (o.isMesh && !o.userData.isSticker) {
        o.castShadow = true
  ...
  useFrame((_, dt) => {
    ...
    // 3b2) 摸猫相位：滚动驱动为主（刮时间轴的手感），待机时缓慢轻抚
    if (petUniforms) {
      petTime.current += dt
    ...
    if (followGroup && !isMobile.current) {
      ...
      followEuler.current.set(
        -smouse.current.y * THREE.MathUtils.degToRad(follow.pitch),
        smouse.current.x * THREE.MathUtils.degToRad(follow.yaw),
        0
      )
      followGroup.quaternion.setFromEuler(followEuler.current)
    }
...
export default function Scene() {
  ...
  return (
    <>
      {/* 铁磁流体背景球（替代旧 GradientBackground 渐变球）：场景内渲染，后处理与不透明画布不受影响 */}
